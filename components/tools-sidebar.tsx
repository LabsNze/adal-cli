...
const codySearchTypes = [
  ...
  { value: "reference", label: "Refs", icon: GitFork },
  { value: "definition", label: "Def", icon: ArrowRight },
  { value: "semantic", label: "Semantic", icon: Layers },
]

const codyInsights = [
  { value: "architecture", label: "Architecture", icon: Network, color: "text-blue-400" },
  { value: "dependencies", label: "Deps", icon: GitFork, color: "text-cyan-400" },
...
const cortexMetricTypes = [
  { value: "dora", label: "DORA Metrics", icon: Activity, color: "text-blue-400", desc: "Deploy frequency, lead time, CFR" },
  { value: "velocity", label: "Velocity", icon: TrendingUp, color: "text-emerald-400", desc: "Cycle time, PR throughput" },
  { value: "quality", label: "Quality", icon: ShieldCheck, color: "text-amber-400", desc: "Defect rate, test coverage" },
  { value: "reliability", label: "Reliability", icon: Gauge, color: "text-red-400", desc: "Uptime, incidents, latency" },
  { value: "developer-experience", label: "Dev Experience", icon: Smile, color: "text-pink-400", desc: "Onboarding, build time, DX" },
]

const augmentActions = [
  { value: "debug", label: "Debug", icon: Bug, color: "text-red-400", desc: "Context-aware debugging" },
  { value: "refactor", label: "Refactor", icon: RefreshCw, color: "text-blue-400", desc: "Extract, simplify, DRY" },
  { value: "conventions", label: "Conventions", icon: Eye, color: "text-cyan-400", desc: "Style compliance" },
  { value: "performance", label: "Performance", icon: Zap, color: "text-amber-400", desc: "Bottleneck optimization" },
  { value: "readability", label: "Readability", icon: Sparkles, color: "text-emerald-400", desc: "Clarity improvements" },
  { value: "modernize", label: "Modernize", icon: Wand2, color: "text-pink-400", desc: "Latest patterns" },
]
...
export function ToolsSidebar({ onAction, isStreaming, messages }: ToolsSidebarProps) {
  ...
  return (
    <div className="p-3 space-y-4 overflow-auto">
        ...
        <div className="space-y-1.5">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-8 bg-secondary/50 border-border min-h-[44px] text-sm"
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault()
                  handleCodySearch()
          ...
          <div className="flex gap-1.5">
            <Select value={searchType} onValueChange={setSearchType} disabled={isStreaming}>
              <SelectTrigger className="bg-secondary/50 border-border min-h-[40px] text-xs flex-1">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {codySearchTypes.map((st) => {
                  ...
                  return (
                    <SelectItem key={st.value} value={st.value} className="min-h-[40px]">
          ...
          <Input
            ...
            placeholder="File filter (*.ts, src/**/*.py)"
            disabled={isStreaming}
            className="bg-secondary/50 border-border min-h-[36px] text-xs text-muted-foreground"
          />
        </div>
        ...
        <div className="space-y-1">
          {[
            ...
            <Button
              ...
              className="h-auto w-full justify-start text-left px-2.5 py-2 text-xs text-foreground/70 hover:text-foreground hover:bg-accent/50 min-h-[40px] active:scale-[0.98]"
            >
            ...
            {codyInsights.map((insight) => {
              ...
              return (
                <Button
                  ...
                  className="h-auto flex-col items-center gap-1 py-2 px-1.5 text-center border-border/50 bg-card hover:bg-accent/50 active:scale-[0.98] transition-all min-h-[52px]"
                >
                  <InsightIcon className={`h-3.5 w-3.5 ${insight.color}`} />
                  <span className="text-[9px] font-medium text-foreground leading-tight">{insight.label}</span>
                </Button>
              )
            })}
          </div>
          ...
          {cortexMetricTypes.map((metric) => {
            ...
            return (
              <Button
                ...
                className="h-auto w-full justify-start text-left px-2.5 py-2 text-xs hover:bg-accent/50 gap-2.5 min-h-[44px] active:scale-[0.98]"
              >
                <MetricIcon className={`h-3.5 w-3.5 shrink-0 ${metric.color}`} />
                <div className="flex flex-col items-start gap-0 min-w-0">
                  <span className="font-medium text-foreground/80 text-xs">{metric.label}</span>
                  <span className="text-[10px] text-muted-foreground leading-tight truncate w-full">{metric.desc}</span>
                </div>
              </Button>
            )
          })}
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              ...
              <Button
                ...
                className="h-auto py-1.5 px-2.5 border-border/50 bg-card hover:bg-accent/50 active:scale-[0.98] transition-all min-h-[32px]"
              >
      ...
      <div className="space-y-2.5">
        <div className="flex items-center gap-2 rounded-lg bg-muted/30 border border-border/50 px-3 py-2">
          <Wand2 className="h-3.5 w-3.5 text-teal-400 shrink-0" />
          ...
          {augmentActions.map((action) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-start gap-0 min-w-0">
                  <span className="font-medium text-foreground/80 text-xs">{action.label}</span>
                  <span className="text-[10px] text-muted-foreground leading-tight truncate w-full">{action.desc}</span>
                </div>
              </Button>
            )
          })}
